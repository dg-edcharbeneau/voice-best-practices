...
export type ModeKey = 'orbits' | 'globe' | 'rubik' | 'wave' | 'ribbon' | 'morph' | 'logo';
...
const PRESETS: Record<ModeKey, Record<OrbSize, Preset>> = {
  ...
  logo: {
    // The dot cloud is a fixed set (logo-points.ts), so `count` stays 1; only
    // the dot radius scales between the two sizes. Speed drives the shimmer.
    64: { speed: 1, count: 1, size: 1 },
    20: { speed: 1, count: 1, size: 1.7 }
  }
};

 ...
 * - `logo`      — the dots are arranged into the Deepgram logomark
 */
...
export interface ThinkingOrbProps extends Omit<CanvasHTMLAttributes<HTMLCanvasElement>, 'style'> {
  ...
  /**
   * When true, the `logo` arrangement rotates a quarter-turn clockwise (eased).
   * No effect on the other states. @default false
   */
  dotActive?: boolean;

  /**
   * Live loudness 0..1 (e.g. TTS output level). Parts the `logo` arrangement's
   * "lips" — a talking-mouth animation that opens with the level and closes to
   * the whole mark when silent. Eased internally. No effect on the other
   * states. @default 0
   */
  energy?: number;

  /**
   * When true, the `logo` arrangement adds two eyes above the mouth — a `:D`
   * face. Intended for the listening/thinking/speaking states. No effect
